...
function NewsletterPopup() {
  ...
  useEffect(() => {
    setTimeout(() => {
      ...
      if (
        currentTime - parseInt(localStorage.getItem("lastVisitTime")) <
        oneWeek
      ) {
        setIsOpen(false)
      } else {
  ...
  return (
    <div
      className={` z-50 flex items-center justify-center fixed top-0 w-screen ${
        !isOpen ? "hidden" : ""
      } h-screen bg-[#00000066]`}
    >
      <div className="max-w-lg md:max-w-none w-11/12 relative z-20 md:w-2/3 lg:w-2/3 bg-white flex items-center mx-auto rounded-md overflow-hidden border shadow-md">
        ...
        <div className="animate__animated animate__fadeInUp py-8 w-full md:w-1/2 h-full flex flex-col justify-center p-4 md:p-8 gap-4 relative">
          ...
          <p className="text-md text-gray-600 font-semibold">
            Subscribe the <span className="text-lg font-semibold">Anon</span> to
            get latest products and discount update.
          </p>
          <input
          ...
          />
          <button className="rounded-md w-full self-center py-2 px-3 bg-red-500 text-white hover:bg-red-600">
            SUBSCRIBE
          </button>
        </div>
        <button
          ...
          className="closeButton absolute top-3 right-3 text-2xl hover:text-red-500"
        >
          <svg
